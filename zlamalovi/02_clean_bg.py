...
def clean_page(input_path: Path, output_path: Path):
    ...
    # Lower C value captures lighter text (more lenient threshold)
    bw = cv2.adaptiveThreshold(
        gray,
        maxValue=255,
        adaptiveMethod=cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
        thresholdType=cv2.THRESH_BINARY,
        blockSize=99,   # size of neighbourhood (smaller = better local adaptation)
        C=5            # negative value = more lenient, captures lighter text
    )
    ...
    # min_size = 25  # pixels; reduced to preserve smaller letters and text components
...
def main():
    ...
    if in_path.is_dir():
        ...
        for img_file in sorted(in_path.iterdir())[0:2]:
